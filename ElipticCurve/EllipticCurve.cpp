...
#include "Field.h"
void EllipticCurve::set(Field::BigInt const& a, Field::BigInt const& b, Field::BigInt const& n)
{
...
Point EllipticCurve::mul(Point const& p, BigInteger::BigInt const& a) const
{
    ...
    for(auto it = (a % (N - "1"_BigInt)) + "1"_BigInt; it > "0"_BigInt; it--)
    {
        res = add(res, p);
    }
    return res;
